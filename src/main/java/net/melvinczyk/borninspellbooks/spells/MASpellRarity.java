package net.melvinczyk.borninspellbooks.spells;
...
public enum MASpellRarity {
    ...
    MASpellRarity(final int newValue) {
        value = newValue;
    }
    ...
    private final static LazyOptional<List<Double>> rawRarityConfig = LazyOptional.of(MASpellRarity::getRawRarityConfigInternal);
     ...
     */
    public int compareRarity(MASpellRarity other) {
    ...
    public static void rarityTest() {
        ...
        SpellRegistry.REGISTRY.get().getValues().forEach(s -> {
            ...
            for (int i = s.getMinRarity(); i <= s.getMaxRarity(); i++) {
                MASpellRarity maRarity = MASpellRarity.values()[i];
                if (maRarity == MASpellRarity.MYTHIC)
                {
                    sb.append(String.format("\t\t%s -> Level %s\n", maRarity, 1));
                }
                else
                {
                    sb.append(String.format("\t\t%s -> Level %s\n", MASpellRarity.values()[i], s.getMinLevelForRarity(SpellRarity.values()[i])));
                }
            }
        });
    ...
    public ChatFormatting getChatFormatting() {
        return switch (this) {
            ...
            case MYTHIC -> ChatFormatting.OBFUSCATED;
        };
    }
