...
public class PumpkinProjectile extends AbstractMagicProjectile {
    public PumpkinProjectile(EntityType<? extends Projectile> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
    }

    public PumpkinProjectile(Level level, LivingEntity shooter) {
        this(MAEntityRegistry.PUMPKIN_PROJECTILE.get(), level);
        setOwner(shooter);
    }
        ...
        for (Entity entity : entities) {
            ...
            if (distance < explosionRadius * explosionRadius && canHitEntity(entity)) {
                if (Utils.hasLineOfSight(level(), hitresult.getLocation(), entity.position().add(0, entity.getEyeHeight() * .5f, 0), true)) {
                    ...
                    DamageSources.applyDamage(entity, damage, MASpellRegistry.PUMPKIN_SMASH.get().getDamageSource(this, getOwner()));
                }
            }
        }
        discard();
